 ...
 * 
 ...
 2. Scan Duration: Period for which the Device with RRM logic should remain in a particular channel while scanning
...
static void Connection (std::string context,Mac48Address bssid)
{
...
void displayStnSnr (Ptr<RriModule> rriMod)
{
		...
		for (it1 = mapapSnrSsid.begin(); it1 != mapapSnrSsid.end(); ++it1)
		{
				...
				cout << setw(20) << it1->first << "\t"  << setw(10) << it1->second.first << " \t \t" << chNo <<"\t" << ssidVal<<"\n";
...
void AssociateWithBestSNR(Ptr<StaWifiMac> staMac1, Ptr<RriModule> rriMod)
{
		...
		for (it1 = mapapSnrSsid.begin(); it1 != mapapSnrSsid.end(); ++it1)
		{
				std::cout<< "MAC Address Of AP: "<< it1->first<<" ; "<< "SINR Value: " << it1->second.first<<std::endl;
				// Check the SNR value
		...
		if(temp_add != staMac1->GetBssid())
		{
				...
				staMac1->GetWifiPhy ()->SetChannelNumber(ch);
...
int  main (int argc, char *argv[])
{
		...
		wifiStaNode.Create (2);  // 2 clients
		...
		msrmac.SetType ("ns3::RriModule",
						"GetLoadUpdates", BooleanValue(false),
		...
		rriMod = DynamicCast<RriModule> (wifiNet->GetMac() );
		...
		positionAlloc1->Add (Vector (100.0, 20.0, 0.0)); // server
		positionAlloc1->Add (Vector (0.0, 10.0, 0.0)); //Initial position of AP1
		positionAlloc1->Add (Vector (25.0, 10.0, 0.0)); //Initial position of the Ap2
		positionAlloc1->Add (Vector (50.0, 10.0, 0.0)); //Initial position of the Ap3
		positionAlloc1->Add (Vector (75.0, 10.0, 0.0)); //Initial position of the Ap4

		positionAlloc1->Add (Vector (37.0, 15.0, 0.0));  //Initial position of client1
		positionAlloc1->Add (Vector (40.0, 15.0, 0.0));//Initial position of client2
		...
		ApplicationContainer sourceApp1,sourceApp2;
		...
		ApplicationContainer sink1,sink2;
		...
		thrpt = totalPacketsThrough*8/(2*1000.0);
		std::cout <<" Thrpt of client 2  "   << " Kbps =  "<< thrpt <<endl;
